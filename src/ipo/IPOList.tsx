...
import { TrendingUp, Plus, RotateCcw, Award, Clock, Loader2, Search, CalendarCheck } from 'lucide-react';
...
type DialogMode = 'apply' | 'allot' | 'refund' | 'update_status' | null;
...
const IPOList: React.FC = () => {
  ...
  // Check for IPOs with allotment date today or past
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Track applications</p>
        </div>
        <Button
          onClick={() => {
            setApplyForm({
      ...
      {pendingAllotments.length > 0 && (
        <div className="p-4 rounded-lg border border-warning bg-warning/10">
          ...
          <p className="text-sm text-muted-foreground mb-3">
            The following IPOs have reached their allotment date. Please update their status:
          </p>
              ...
              >
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Allotment Date: {formatDate(ipo.allotment_date!)}
                  </p>
                </div>
                <Button size="sm" onClick={() => openUpdateStatusDialog(ipo)}>
                  Update Status
                </Button>
              </div>
            ))}
      ...
      <div className="space-y-4">
        {filteredIPOs.length === 0 ? (
          ...
          filteredIPOs.map((ipo) => {
            ...
            const isAllotmentDue =
              ipo.status === 'APPLIED' &&
              ipo.allotment_date &&
              ipo.allotment_date <= getTodayIST();

            return (
              <div
                key={ipo.id}
                className={`card-finance p-5 flex flex-col lg:flex-row lg:items-center gap-4 ${
                  isAllotmentDue ? 'ring-2 ring-warning' : ''
                }`}
              >
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full border ${status.class}`}
                    >
                      {status.label}
                    </span>
                    {isAllotmentDue && (
                      <span className="text-xs px-2 py-0.5 rounded-full bg-warning text-warning-foreground">
                        Allotment Due
                      </span>
                    )}
                  </div>
                  <p className="text-sm text-muted-foreground">
                    ...
                    {ipo.allotment_date && (
                      <> • Allotment: {formatDate(ipo.allotment_date)}</>
                    )}
                    {ipo.shares_allotted != null && (
                      <> • Got: {ipo.shares_allotted} shares</>
                    )}
                  </p>
                </div>
                ...
                {ipo.status === 'APPLIED' && (
                  <div className="flex gap-2">
                    {isAllotmentDue ? (
                      <Button
                        onClick={() => openUpdateStatusDialog(ipo)}
                        size="sm"
                        className="gap-1"
                      >
      ...
      <Dialog open={dialogMode === 'apply'} onOpenChange={() => setDialogMode(null)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Enter the IPO details and expected allotment date</DialogDescription>
          </DialogHeader>
          <form onSubmit={handleApplySubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input
                  type="date"
                  value={applyForm.allotment_date}
                  onChange={(e) =>
                    setApplyForm({ ...applyForm, allotment_date: e.target.value })
                  }
                  required
                />
              </div>
      ...
      <Dialog open={dialogMode === 'update_status'} onOpenChange={() => setDialogMode(null)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedIPO?.company_name} - Allotment date was {selectedIPO?.allotment_date && formatDate(selectedIPO.allotment_date)}
            </DialogDescription>
          </DialogHeader>
          <div className="p-4 rounded-lg bg-muted/50 flex justify-between mb-4">
            <div>
              <p className="text-sm text-muted-foreground">Applied</p>
              <p className="font-semibold">{selectedIPO?.shares_applied} shares</p>
            </div>
            <div className="text-right">
              <p className="text-sm text-muted-foreground">Amount</p>
              <p className="font-semibold mono">{formatMoney(selectedIPO?.amount)}</p>
            </div>
          </div>
          <p className="text-center text-muted-foreground mb-4">
            Were you allotted shares in this IPO?
          </p>
      ...
      <Dialog open={dialogMode === 'allot'} onOpenChange={() => setDialogMode(null)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Enter the number of shares allotted and any refund amount</DialogDescription>
          </DialogHeader>
          <form onSubmit={handleAllotSubmit} className="space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Enter refund amount if partial allotment
              </p>
            </div>
            <div className="flex gap-3 pt-4">
              <Button
              ...
              <Button type="submit" disabled={saving} className="flex-1">
                {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Record Allotment'}
              </Button>
            </div>
          </form>
        </DialogContent>
      ...
      <Dialog open={dialogMode === 'refund'} onOpenChange={() => setDialogMode(null)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Process full refund for this IPO application</DialogDescription>
          </DialogHeader>
          ...
          <p className="text-muted-foreground">
            This will refund the full amount to your bank account.
          </p>
          <div className="flex gap-3 pt-4">
            <Button
              variant="outline"
            ...
            <Button onClick={handleRefund} disabled={saving} className="flex-1">
